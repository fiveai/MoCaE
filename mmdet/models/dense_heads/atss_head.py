...
class ATSSHead(AnchorHead):
    ...
    def __init__(self,
        ...
        self.log_folder = "calibration/atss/detections/val/"
        self.log_folder = "calibration/dyhead/detections/val/"
        self.nms_time = 0
        '''
        calibration_file = "calibration/atss/calibrators/IR_class_agnostic500.pkl"
    ...
    def _bbox_post_process(self,
        ...
        with open(filename, "a") as f:
            x = np.concatenate((mlvl_nms_scores[:, :-1].cpu().numpy(), mlvl_bboxes.cpu().numpy()), axis=1)
            np.save(filename, x)
        self.log_file_counter += 1
        '''

        #s_ = self.calibrator[0].predict(mlvl_nms_scores[:, :-1].cpu().numpy().reshape(-1, 1))
        #mlvl_nms_scores[:, :-1] = torch.from_numpy(s_.reshape(-1, self.num_classes)).cuda().type(torch.cuda.FloatTensor)

        

        det_bboxes, det_labels, nms_time = multiclass_nms(
            mlvl_bboxes,
            mlvl_nms_scores,
            cfg.score_thr,
            cfg.nms,

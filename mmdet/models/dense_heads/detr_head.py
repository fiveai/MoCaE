...
class DETRHead(AnchorFreeHead):
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            bbox_pred_ = bbox_pred[bbox_index] 
        else:
        ...
        det_bboxes = bbox_cxcywh_to_xyxy(bbox_pred_)
        ...
        det_bboxes_ = bbox_cxcywh_to_xyxy(bbox_pred)
        det_bboxes_[:, 0::2] = det_bboxes_[:, 0::2] * img_shape[1]
        det_bboxes_[:, 1::2] = det_bboxes_[:, 1::2] * img_shape[0]
        det_bboxes_[:, 0::2].clamp_(min=0, max=img_shape[1])
        det_bboxes_[:, 1::2].clamp_(min=0, max=img_shape[0])
        if rescale:
            det_bboxes_ /= det_bboxes_.new_tensor(scale_factor)

        if os.path.exists(filename):
            os.remove(filename)
        with open(filename, "a") as f:
            x = np.concatenate((cls_score.cpu().numpy(), det_bboxes_.cpu().numpy()), axis=1)
            np.save(filename, x)

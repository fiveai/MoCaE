...
class RandomCorrupt:
    ...
    def __init__(self, corruptions, severity=1):
    ...
    def __call__(self, results):
        ...
        corruption = self.corruptions[random.randint(0, self.num_corruptions-1)]
        results['img'] = corrupt(
            results['img'].astype(np.uint8),
...
class RemoveObjects:
    ...
    def __init__(self,
                 fill='constant', # constant or crop
                 fill_in=(0, 0, 0)):
    ...
    def __call__(self, results):
        ...
        plt.imshow(mmcv.bgr2rgb(results['img']))
        plt.show()
        '''

        for gt_box in results['gt_bboxes']:
            results['img'][int(gt_box[1]):int(gt_box[3]), int(gt_box[0]):int(gt_box[2]), :] = self.fill_in
        '''
        plt.imshow(mmcv.bgr2rgb(results['img']))
        plt.show()
        '''
...
class OODMixUp:
    ...
    def __init__(self,
        ...
        self.num_corruptions = len(self.corruptions)
        self.idx = 0
        self.OFFSET = 4000000 + offset_ctr*1000000
    ...
    def _crop(self, img, min_dim=32):
        h, w = img.shape[:2]
        x_offset, y_offset = 0, 0
        h_ratio = random.uniform(*self.ratio_range)
        w_ratio = random.uniform(*self.ratio_range)
        target_h = random.randint(int(img.shape[0]*h_ratio), img.shape[0]) 
        target_w = random.randint(int(img.shape[1]*w_ratio), img.shape[1])
        if target_h > min_dim or target_w > min_dim:
            return img[y_offset:y_offset + target_h, x_offset:x_offset + target_w]
        return img
    ...
    def _mixup_transform(self, results):
        ...
        ori_img = results['img']
        mix_img = results['mix_results'][0]['img']
        ''' 
        import matplotlib.pyplot as plt
        img = mmcv.bgr2rgb(ori_img)
        plt.imshow(img)
        plt.savefig(str(self.idx) + '.png')
        plt.clf()
        self.idx +=1 

        img = mmcv.bgr2rgb(mix_img)
        plt.imshow(img)
        plt.savefig(str(self.idx) + '.png')
        plt.clf()
        self.idx +=1 
        ...
        # 1. flip
        is_flip_ori = random.uniform(0, 1) > self.flip_ratio
        if is_flip_ori:
            ori_img = ori_img[:, ::-1, :]

        is_flip_mix = random.uniform(0, 1) > self.flip_ratio
        if is_flip_mix:
            mix_img = mix_img[:, ::-1, :]

        # 2. random crop
        ori_img = self._crop(ori_img)
        mix_img = self._crop(mix_img)

        # 3. corrupt
        if self.corruptions is not None:
            h, w, _ = ori_img.astype(np.uint8).shape
            if h > 32 and w>32:
                corruption = self.corruptions[random.randint(0, self.num_corruptions-1)]
            ...
            h, w, _ = mix_img.astype(np.uint8).shape
            if h > 32 and w>32:
                corruption = self.corruptions[random.randint(0, self.num_corruptions-1)]
                severity = random.randint(*self.severity_range)
                mix_img= corrupt(
        ...
        # Resize image such that they have equal sizes 
        mix_img = mmcv.imresize_like(mix_img, ori_img, return_scale=False)

        # 8. mix up
        ori_img = ori_img.astype(np.float32)
        lam = np.random.beta(10, 10)

        mixup_img = lam * ori_img + (1 - lam) * mix_img.astype(np.float32)

        results['img'] = mixup_img.astype(np.uint8)
        results['ori_shape'] = results['img_shape'] = mixup_img.shape
        results['img_info']['height'] = mixup_img.shape[0]
        results['img_info']['width'] = mixup_img.shape[1]
        results['img_info']['id'] = self.OFFSET + self.idx
        self.idx += 1
        return results
    ...
    def __repr__(self):
        ...
        repr_str += f'ratio_range={self.ratio_range}, '
        repr_str += f'flip_ratio={self.flip_ratio}, '
        repr_str += f'corruptions={self.corruptions}, '
        repr_str += f'severity_range={self.severity_range}, '
        return repr_str
